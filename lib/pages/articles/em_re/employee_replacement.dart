...
import '../../upload/emplyee_replacement/upload.dart';

class EmployeeReplacementArticles extends StatefulWidget {
  final String category;
  final String categoryImage;
  final String tag;

  EmployeeReplacementArticles(
      {Key? key,
  ...
  @override
  _EmployeeReplacementArticlesState createState() =>
      _EmployeeReplacementArticlesState();
}

class _EmployeeReplacementArticlesState
    extends State<EmployeeReplacementArticles> {
  final FirebaseFirestore firestore = FirebaseFirestore.instance;
  final String collectionName = 'emp_replace';
  ScrollController? controller;
  DocumentSnapshot? _lastVisible;
  late bool _isLoading;
  List<DocumentSnapshot> _snap = [];
  List<EmployeeReplacement> _data = [];
  bool? _hasData;
  ...
  // checkIsAdmin() {
  //   AppService()
  //       .checkAdminAccount(FirebaseAuth.instance.currentUser!.uid)
  //       .then((value) {
  //     if (value != null) {
  //       _isAdmin = value;
  //     }
  ...
  String _getCollection() {
    if (initialCategories[6] == widget.category) {
      return "sponsors";
    } else if (initialCategories[1] == widget.category) {
      return "offers";
    } else if (initialCategories[2] == widget.category) {
      return "cele";
    } else if (initialCategories[3] == widget.category) {
      return "medical_jobs";
    } else if (initialCategories[4] == widget.category) {
      return "emp_replace";
    } else if (initialCategories[5] == widget.category) {
      return "emp_replace";
    } else {
      return "content";
    }
    ...
    if (data.docs.length > 0) {
      ...
      if (mounted) {
        setState(() {
          ...
          _data =
              _snap.map((e) => EmployeeReplacement.fromFirestore(e)).toList();
        });
      }
    } else {
      if (_lastVisible == null) {
      ...
      floatingActionButton: FloatingActionButton(
        onPressed: () {
          Navigator.of(context).push(
            MaterialPageRoute(
              builder: (context) {
                return UploadEmployeeReplacement();
              },
            ),
          );
        },
